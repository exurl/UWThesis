...
\begin{table}[h]
	...
	\end{tabular}
\end{table}
Note that Table \ref{tab:tuningParams} applies for the case of $N_s=2$ structural dynamic modes used in modeling. When a higher number of modes is used, $\zeta_i$ continues to take the values of damping ratio results from GVT (found in Table \ref{tab:expModalData}) and the Roger matrices $[P_i]$ are a larger matrix of ones.
...
\begin{figure}[H]
	...
	\caption{A comparison of experimental FRFs and untuned model FRFs. The grey region is enclosed by the experimental $H_1$ FRF below and the experimental $H_2$ FRF above.}
\end{figure}
...
A summary of the results of several model tuning optimization studies are shown in Table \ref{tab:optResult}. 
