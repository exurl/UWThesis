...
\begin{table}[h]
	...
	\begin{tabular}{ccc}
		...
		$\left[\tau_{P_{ss1}}\right]$ & all ones & scaling lift from state \\
		$\left[\tau_{P_{ss2}}\right]$ & all ones & scaling lift from state rates \\
		$\left[\tau_{P_{ss3}}\right]$ & all ones & scaling lift from state accelerations \\
		\hline\hline
	\end{tabular}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{ccc}
		...
		$\tau_\text{elev}$ & 1 & 0.6 \\
		$\tau_\text{gust}$ & 1 & N/A \\
		$\left[\tau_{P_{ss1}}\right]$ & all ones & $\begin{bmatrix} 0.9 & 1 \\ 0.5 & 1.5 \end{bmatrix}$ \\
		$\left[\tau_{P_{ss2}}\right]$ & all ones & $\begin{bmatrix} 0.9 & 1 \\ 0.5 & 1.5 \end{bmatrix}$ \\
		$\left[\tau_{P_{ss3}}\right]$ & all ones & $\begin{bmatrix} 0.9 & 1 \\ 0.5 & 1.5 \end{bmatrix}$ \\
		\hline\hline
	\end{tabular}
\end{table}
